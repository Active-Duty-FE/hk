<template>
  ...
  >
    <h1 class="font-550 mb-10 text-center text-[28px] leading-7">Shipping</h1>

    <div v-clean-html="dataShipping?.contents" class="!text-base !leading-[22px]" />
    <p class="mb-0 w-full text-start text-xs">
      Update : {{ formatDateTime(dataShipping.exposureStartDate, FORMAT_FULL_YEAR_USE_DOT) }}
    </p>
  </div>
</template>
<script setup>
...
const { mutate: mutationGetShipping } = useMutation(
  ...
  },
  {
    onSuccess: (response) => {
      ...
      if (response.success) Object.assign(dataShipping, response.data)
    }
  }
)
...
onMounted(() => {
  const params = {
    ...
    termsUseType: TERMS_USE_TYPE.SHIPPING
  }
