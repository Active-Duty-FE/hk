<template>
  <div class="inquiryForm">
    <a-form ref="formRef" :model="formState" :rules="rules" class="w-full" @finish="onSubmit">
      ...
      <div class="inquiryForm__upload">
        ...
        <div class="inquiryForm__upload-files">
          ...
          >
            ...
            />
            <img
              :src="inquiryId && item?.id ? addPrefixForImageInquiry(item.url) : item.url"
              alt=""
      ...
      <div class="!pt-[30px]">
        <RLabel title="Cell Phone Number" required />
      ...
      <div>
        <RButton
          html-type="submit"
          title="Submit"
          class="!bg-navy-6 mx-auto my-10 w-full !border-none md:w-[335px]"
  ...
  />
  <RModalAlert
    v-model:visible="formState.isShowModalOrder"
    :width="500"
    :footer="false"
    :mask-closable="true"
    :closable="true"
    @cancel="formState.isShowModalOrder = false"
  >
...
const onChangeFiles = ({ file }) => {
  ...
  if (!inquiryId && formState.files.length >= 3) return
  if (inquiryId && formState.filesPreview.length >= 3) return
...
const onRemoveFile = (index, item) => {
  formState.filesPreview.splice(index, 1)
  formState.files.splice(index, 1)
  if (inquiryId && item?.id) {
    formState.deleteInquiryAttachmentList.push(item.id)
  }
}
...
const { mutate: editInquiry } = useMutation(
  async ({ payload, id }) => {
    return await apiService.conciergeService.editInquiry(payload, id)
  },
  {
    onSuccess: (data) => {
      if (data?.success && data?.data) {
...
const onSubmit = async () => {
  try {
    ...
    if (!formState.filesPreview.length) {
      formState.descriptionModal = CONCIERGE_MESSAGE.image
      formState.isShowModalConfirm = true
      return
    }
    ...
    if (salesOrderNumber.value && salesOrderNumber.value !== 'null') {
      submitData.salesOrderNumber = salesOrderNumber.value
    }
    ...
    if (inquiryId && formState.deleteInquiryAttachmentList.length) {
      for (let i = 0; i < formState.deleteInquiryAttachmentList.length; i++) {
        const fileId = formState.deleteInquiryAttachmentList[i]
        payload.set(`deleteInquiryAttachmentList[${i}]`, fileId)
      }
    }

    inquiryId ? editInquiry({ payload, id: inquiryId }) : registerInquiry(payload)
  } catch (error) {
    showGlobalLoading(false)
  }
}
...
const getDetailInquiry = async (id) => {
  showGlobalLoading(true)
  const response = await apiService.conciergeService.detailInquiry(id)
  showGlobalLoading(false)
  if (response?.success && response?.data) {
    formState.inquiryDetail = response.data
    formState.type = formState.inquiryDetail?.inquiryType?.text
    formState.title = formState.inquiryDetail?.inquiryTitle
    formState.content = formState.inquiryDetail?.inquiryContents
    formState.cellPhone = formState.inquiryDetail?.telNo || undefined
    formState.filesPreview = formState.inquiryDetail?.inquiryAttachments || []
    salesOrderNumber.value =
      response?.data?.salesOrderNumber && response?.data?.salesOrderNumber !== 'null'
        ? response?.data?.salesOrderNumber
        : null
    orderState.orderNumber = salesOrderNumber.value
    formState.formType = salesOrderNumber.value
      ? InquiryFormType.ORDER_INQUIRY
      : response?.data?.inquiryFormType?.code || InquiryFormType.GENERAL_INQUIRY
  }
}
